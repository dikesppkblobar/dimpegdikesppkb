  ...
  return (
    <div className="bg-[#16161a] border border-white/5 p-6 rounded-2xl shadow-xs space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex border-b border-white/5 -mx-6 px-6 overflow-x-auto whitespace-nowrap">
        ...
        <button
          onClick={() => setActiveMgtTab('branding')}
          className={`pb-3 text-xs font-bold transition flex items-center space-x-1.5 border-b-2 mr-6 ${activeMgtTab === 'branding' ? 'border-emerald-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
        >
          <ImageIcon size={15} className="text-yellow-500" />
          <span className="text-yellow-600">Identitas Visual</span>
        </button>
      ...
      {/* ======================= SUB-TAB: BRANDING IDENTITAS VISUAL ======================= */}
      {activeMgtTab === 'branding' && (
        <div className="space-y-6 pt-2 text-slate-800 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm text-left space-y-4">
              <h3 className="text-base font-bold text-slate-900 font-display flex items-center gap-2">
                ...
                <span>Konfigurasi Logo & Favicon Dinas</span>
              </h3>
              <p className="text-xs text-slate-500">
                Sesuaikan aset gambar logo dan favicon ikon peramban untuk seluruh sistem SIMPEG Lombok Barat.
              </p>

              <form onSubmit={handleSaveBranding} className="space-y-4 pt-2">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">URL / Path Logo Utama</label>
                  <input
                    type="text"
                    value={inputLogo}
                    onChange={(e) => setInputLogo(e.target.value)}
                    placeholder="Contoh: /logo_lombok_barat.png"
                    className="w-full px-3.5 py-2 border border-slate-200 bg-white rounded-lg text-xs"
                    required
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Aset internal default: <code className="font-mono bg-slate-50 px-1 py-0.5 rounded">/logo_lombok_barat.png</code></p>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">URL / Path Favicon (Browser Icon)</label>
                  <input
                    type="text"
                    value={inputFavicon}
                    ...
                    required
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Gunakan tautan icon yang valid atau sesuaikan dengan logo utama.</p>
                </div>

                <div className="flex justify-end pt-2">
                  <button
                    type="submit"
                    className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-slate-900 rounded-lg text-xs font-bold transition flex items-center space-x-1.5 shadow-sm cursor-pointer"
                  >
                    <CheckSquare size={14} />
                    <span>Terapkan & Simpan Identitas</span>
                  </button>
                </div>
              </form>

              {/* REAL-TIME VISUAL PRATINJAU */}
              <div className="mt-6 border-t border-slate-100 pt-5 space-y-3">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Pratinjau Hasil Penerapan</h4>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-250 flex items-center space-x-4">
                  <div className="p-1.5 bg-white rounded-xl border border-slate-200 shadow-xs shrink-0">
                    <img
                      src={inputLogo || "/logo_lombok_barat.png"}
                      onError={(e) => {
                  ...
                  <div>
                    <h4 className="text-sm font-display font-bold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                      <span>SIMPEG DINKES PPKB</span>
                      <span className="text-[9px] bg-emerald-100 text-emerald-800 border border-emerald-200 px-1.5 py-0.5 rounded-full uppercase">LOBAR</span>
                    </h4>
                    <p className="text-[10.5px] text-slate-500 font-semibold">Sistem Layanan Kepegawaian & Penataan SDMK Terintegrasi</p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm text-left space-y-4">
              <h3 className="text-base font-bold text-slate-900 font-display flex items-center gap-2">
                ...
                <span>Katalog Preset Tema Visual Resmi</span>
              </h3>
              <p className="text-xs text-slate-500">
                Pilih atau edit salah satu opsi preset visual instansi di bawah untuk mengubah tampilan sistem dalam satu klik.
              </p>
              ...
              <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                {brandingPresets.map((p) => (
                  <div
                    key={p.id}
                    className={`p-4 rounded-xl border transition flex items-center justify-between gap-4 ${p.is_active ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200 bg-white hover:bg-slate-50/50'}`}
                  >
                    <div className="flex items-center space-x-3 min-w-0">
                      <div className="h-10 w-10 rounded-lg border border-slate-200 p-1 bg-white shrink-0 flex items-center justify-center">
                        <img src={p.logo || "/logo_lombok_barat.png"} alt={p.name} className="h-8 w-8 object-contain" referrerPolicy="no-referrer" />
                      </div>
                      <div className="truncate">
                        <p className="text-xs font-bold text-slate-950 truncate">{p.name}</p>
                        <p className="text-[10px] text-slate-500 truncate font-mono mt-0.5">{p.logo}</p>
                      </div>
                    </div>

                    <div className="flex items-center space-x-2 shrink-0">
                      {p.is_active ? (
                        <span className="bg-emerald-100 text-emerald-800 border border-emerald-200 text-[10px] font-bold px-2 py-1 rounded-full uppercase">Aktif</span>
                      ) : (
                        <button
                          onClick={() => handleApplyPreset(p.id)}
                          className="bg-slate-50 hover:bg-slate-100 border border-slate-300 text-slate-700 hover:text-slate-900 px-2.5 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer"
                        >
                          Terapkan
                        </button>
                      )}
                      ...
                      <button
                        ...
                        className="p-1 hover:bg-rose-50 text-slate-550 hover:text-rose-600 rounded transition cursor-pointer"
                        title="Hapus Preset"
                      >
                        <Trash2 size={13} />
                      </button>
              ...
              {/* Form Tambah/Ubah Preset */}
              <div className="border-t border-slate-100 pt-4 space-y-3">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">{editingPresetId !== null ? "Ubah Preset Tema" : "Tambah Preset Tema Kustom"}</h4>
                
                <form onSubmit={handleAddPreset} className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="md:col-span-2">
                    <input
                      type="text"
                      value={newPresetName}
                      onChange={(e) => setNewPresetName(e.target.value)}
                      placeholder="Nama Preset Tema (e.g. Tema Puskesmas Gunungsari)"
                  ...
                  <div>
                    <input
                      type="text"
                      value={newPresetLogo}
                      onChange={(e) => setNewPresetLogo(e.target.value)}
                      placeholder="URL Logo (e.g. /logo_lombok_barat.png)"
                      className="w-full px-3 py-1.5 border border-slate-200 bg-white rounded-lg text-xs"
                      required
                    />
                  </div>
                  <div>
                    <input
                      type="text"
                      value={newPresetFavicon}
                      onChange={(e) => setNewPresetFavicon(e.target.value)}
                  ...
                  <div className="md:col-span-2 flex justify-end space-x-2 pt-1">
                    {editingPresetId !== null && (
                      <button
                        ...
                        className="px-3 py-1.5 border border-slate-200 hover:bg-slate-100 text-slate-700 text-xs rounded-lg transition font-semibold cursor-pointer"
                      >
                        Batal
                      </button>
                    )}
                    <button
                      type="submit"
                      className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-950 text-white font-bold rounded-lg text-xs transition flex items-center space-x-1 cursor-pointer"
                    >
                      <Plus size={13} />
                      <span>{editingPresetId !== null ? "Simpan Perubahan" : "Simpan Preset"}</span>
                    </button>
                  </div>
                </form>
